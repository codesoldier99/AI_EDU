...
class KPMatchAgent(Agent):
    ...
    def _expand_prereqs(self, hits: list[Candidate], source_ref: str) -> list[Candidate]:
        ...
        实测（拿导师已标注的 169 条映射当标准答案，图谱 463 个知识点）：
        只往上追一层，Top-6 召回率从 44.4% 提到 59.2%，每个任务平均只多两条候选。
        追两层到 62.1% 但候选数涨到 9 条，队列变长而收益递减，不划算。

        一个必须说清的现象：图谱从 284 个点长到 463 个点之后，同一套算法的召回率
        从 64.3% 掉到 59.2%。不是算法退步了，是候选空间变大、标准答案也变多了。
