"""确定性 PPTX 引擎（纯标准库，不依赖 python-pptx / LibreOffice / 任何外部二进制）。

从 scripts/make_deck.py 抽取而来——那个脚本已经证明了这套手写 OOXML 的写法能生成
可以被 PowerPoint/WPS 正常打开的文件，本模块把它变成 packages/courseware 的共享引擎：
既是 officecli_render.py 在外部渲染器不可用时的降级路径，也是 scripts/make_deck.py
自身现在依赖的实现（避免同一段 OOXML 拼装逻辑存在两份、后续改一处忘一处）。

内容与渲染分离：本模块只认 `spec: dict`（kind/title/bullets/table/...），
不关心这些字段是怎么算出来的——那是 packages/courseware/deck.py 的职责。
"""
