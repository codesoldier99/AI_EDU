...
class TestContentChatAgent(OfflineLLMMixin, DBTestCase):
    """本组用例测的就是**离线降级模式**的行为，因此把模型钉死在离线表达器上。

    原先靠"测试环境没配 Key"来间接得到 degraded=True——一旦开发机配了真实 Key
    就会红，而红的原因跟被测行为毫无关系。断言环境不如固定环境。
    """
    ...
    def test_refine_slide_in_degraded_mode_keeps_original_bullets_unfragmented(self):
        ...
        self.assertTrue(r["degraded"])  # OfflineLLMMixin 已把模型钉在离线端
