...
PracticeBinding(project_code, course_id) # 项目 → 集中实践课程：项目专有知识的学分载体
                                         # 一个项目只能绑一门，否则同一份工作认两次学分
...
| 项目专有知识 | project-only KP | 不属于任何理论课的知识点；只经集中实践环节折算学分 |
| 前置闭包 | prereq closure | 建课时只建被拉取知识点 + 它在本课程内的前置，不铺整本教材 |
...
- ❌ 把项目专有知识硬塞进某门理论课凑覆盖度（硬凑一次，认定的可信度就没了）
- ❌ 一个项目绑两门集中实践课（同一份工作会被认两次学分）
- ❌ 只凭知识点覆盖度就判定集中实践环节合格（里程碑验收必须教师签字，系统不代签）
- ❌ 新增课程图谱文件却不声明 `modules:` 就以为万事大吉（能力模块按代码全局解析，
  但未知代码会被丢弃并告警——见 seed_course 的 unknown_modules）
...
知识点自动匹配（确定性检索 + 依赖边扩展，实测 Top-6 召回 59.2%）与教师审核队列；
...
**Phase 4.3（已完成）：** 按需求队列建课并打通学分归集。照队列顺序补建 8 门课
（具身感知 / 计算机组成原理 / 概率论 / 数据库 / 线性代数 / 面向对象 / 算法 / Python），
**179 个知识点、队列清空**；每门课只建"被拉取知识点在本课程内的前置闭包"，
自然落在 21–24 个点，刚越过认定门槛。新增**集中实践环节绑定**：
DAC 的 97 个项目专有知识经「专业综合实习 G18Z41029」折算，一个项目只能绑一门。
实测：全图 463 知识点 / 547 边，DAC 触及 12/28 门课、拉取 232 个知识点。
副作用如实记录：图谱变大后自动匹配 Top-6 召回从 64.3% 降到 59.2%——
候选空间变大，教师审核这一关只会更重要。
